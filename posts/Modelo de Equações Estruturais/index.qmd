...
```{r}
#| echo: true
fit1lm <- lm(y1 ~ x1 + x2, data=PoliticalDemocracy)
summary(fit1lm)
```
...
summary(fit2)
```
O diagrama de trajetória nesse caso fica:
...
Para rodar um modelo de mensuração (exclusivamente): 
```{r}
m3 <- '
      ind60 =~ x1 + x2 + x3
      '
...
:::{.callout-note}
## Atividade
**Compute o TLI e o CFI na mão. Compara com os resultados obtidos no 'lavann'** 
:::
