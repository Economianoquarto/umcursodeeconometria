...
É uma variável que **existe concretamente no seu banco de dados**, ou seja, foi diretamente medida ou coletada. Também é chamada de **item** (quando é uma pergunta de um questionário, por exemplo) ou **variável manifesta**.

> *Exemplo: A resposta a uma pergunta específica como "Qual sua nota em matemática?" ou "De 1 a 5, quão satisfeito você está?".*

---

### Variável Latente (Latent Variable)

É um **conceito ou construto abstrato que não pode ser medido diretamente** e, portanto, não existe como uma única coluna no seu banco de dados. Ela é "construída" ou inferida a partir de um conjunto de variáveis observadas.

> *Exemplo: "Inteligência", "Ansiedade" ou "Satisfação no Trabalho". Ninguém mede "inteligência" com uma única pergunta; ela é inferida a partir das respostas a vários testes.*

---

### Variável Exógena (Exogenous Variable)

É uma **variável independente** no modelo. Ela funciona como uma "causa" ou preditora e não é explicada por nenhuma outra variável dentro do modelo. Pode ser tanto uma variável observada (`x`) quanto uma latente.

> Pense nela como o *ponto de partida* de uma seta causal.

---

### Variável Endógena (Endogenous Variable)

É uma **variável dependente** no modelo. Ela funciona como um "efeito" ou resultado, pois sempre há pelo menos uma seta causal apontando para ela, indicando que ela é explicada por outras variáveis no modelo. Pode ser tanto uma variável observada (`y`) quanto uma latente.

> Pense nela como o *destino* de uma seta causal.

---

### Modelo de Mensuração (Measurement Model)

É a parte do modelo SEM que **conecta as variáveis observadas (os itens) às suas respectivas variáveis latentes (os conceitos)**. Essencialmente, ele define como os conceitos abstratos são medidos.

> *É aqui que você especifica que as perguntas Q1, Q2 e Q3 são os indicadores do conceito "Ansiedade".*

---

### Indicador (Indicator)

É simplesmente o nome que se dá a uma **variável observada quando ela está sendo usada para medir uma variável latente** dentro de um modelo de mensuração.

> *Se a variável observada "nota em geometria" é usada para medir a variável latente "habilidade matemática", então "nota em geometria" é um indicador.*

---

### Fator (Factor)

É outro nome para uma **variável latente**, especialmente no contexto do modelo de mensuração. Um fator é o conceito abstrato que é "definido" pelo seu conjunto de indicadores.

> *"Inteligência" é o fator que é medido pelos indicadores (respostas a diferentes testes de QI).*

---

### Carga Fatorial (Loading)

É o caminho (a seta) que **liga um indicador (variável observada) ao seu fator (variável latente)**. O valor dessa carga representa a força da relação entre o indicador e o conceito que ele está medindo.

> *Uma carga fatorial alta significa que aquele item é um ótimo medidor do conceito.*

---

### Modelo Estrutural (Structural Model)

É a parte do modelo SEM que **especifica as relações de causa e efeito entre as variáveis**, sejam elas latentes ou observadas. Ele representa as hipóteses principais da sua pesquisa.

> *É aqui que você define as setas que conectam as variáveis exógenas (causas) às endógenas (efeitos), como "Ansiedade → Desempenho Acadêmico".*

---

### Caminho de Regressão (Regression Path)

É a seta que representa uma **relação de causa e efeito direta entre uma variável exógena e uma endógena** dentro do modelo estrutural. Essencialmente, é a sua hipótese de regressão.
...
-   **Modelo de Mensuração**: É a parte do SEM que **liga variáveis observadas a variáveis latentes** \[7, 21, 34, 35\]. Sua função é mensurar construtos latentes a partir de variáveis observadas . Envolve a **Análise Fatorial Confirmatória (AFC)**.
